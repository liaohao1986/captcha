...
public class ImagePreProcess_189 {
	...
	public static void downloadImage() {
		...
		for (int i = 0; i < 30; i++) {
			getMethod = new GetMethod("http://sc.189.cn/kaptcha.jpg");
			try {
				// ִ��getMethod
	...
	public static void trainData() throws Exception {
		File dir = new File("temp_189");
		File[] files = dir.listFiles();
		for (File file : files) {
			BufferedImage img = removeBackgroud("temp_189\\" + file.getName());
			List<BufferedImage> listImg = splitImage(img);
			if (listImg.size() == 4) {
				for (int j = 0; j < listImg.size(); ++j) {
					ImageIO.write(listImg.get(j), "JPG", new File("train_189\\"
